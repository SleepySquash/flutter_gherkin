...
class TodoRepository {
  ...
  final Subject<SharedPreferences> _storage = ReplaySubject<SharedPreferences>(
    maxSize: 1,
  );

  TodoRepository() {
    StreamSubscription<SharedPreferences>? sub;
    sub = SharedPreferences.getInstance().asStream().listen((sp) {
      _storage.add(sp);
      sub?.cancel();
    });
  }
  ...
  Stream<bool> delete(Todo item) {
    return all().switchMap((items) {
      final itemsAsJson = (items.toList()..removeWhere((x) => x.id == item.id))
          .map((x) => jsonEncode(x.toJson()))
          .toList(growable: false);
